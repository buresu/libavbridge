...
static bool is_optional_runtime_unsupported(const char *error) {
    ...
    return std::strstr(error, "not implemented") ||
           std::strstr(error, "not supported") ||
           std::strstr(error, "unsupported") ||
           std::strstr(error, "cannot");
}
...
int main(int argc, char *argv[]) {
    ...
    if (avb_decoder_open(&dec, in_path, &dopts) != AVB_OK) {
        printf("SKIP: native decoder open failed for %s: %s\n", argv[3],
    ...
    if (avb_decoder_get_media_info(dec, &mi) != AVB_OK || !mi.video.available) {
        printf("SKIP: no video stream for native smoke\n");
        avb_decoder_close(dec);
        return AVB_TEST_SKIP;
    }
    ...
    printf("native first frame: %dx%d pts=%.6f device=%d handle=%p\n",
           first.width, first.height, first.pts_sec, (int)first.hardware_device,
           first.native_handle);
    ...
    if (avb_encoder_open(&enc, out_path, &eopts) != AVB_OK) {
        printf("SKIP: native encoder open failed for %s: %s\n", argv[4],
    ...
    while (have_frame) {
        ...
        if (avb_encoder_write_video(enc, &frame, frame.pts_sec) != AVB_OK) {
            ...
            if (written == 0 && is_optional_runtime_unsupported(error)) {
                printf("SKIP: native encoder write unsupported for %s: %s\n",
    ...
    if (written == 0) {
        printf("SKIP: no native frames written\n");
        avb_encoder_close(enc);
        return AVB_TEST_SKIP;
    }
    ...
    printf("native roundtrip passed: %s -> %s, %d frames\n", argv[3], argv[4], written);
    return 0;
}
