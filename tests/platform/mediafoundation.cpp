...
static int smoke_audio_decoder(const char *path, avb_audio_codec codec,
  ...
  if (avb_decoder_probe_runtime_capabilities(AVB_BACKEND_MEDIAFOUNDATION, path,
                                             &caps) != AVB_OK ||
  ...
  std::printf(
      "Media Foundation %s decode smoke passed (%d frames, codec '%s')\n",
...
static int smoke_video_decoder(const char *path, avb_video_codec codec,
  ...
  if (read != AVB_OK) {
    std::fprintf(
        stderr, "read %s produced no video frame (%d): %s\n", path, (int)read,
        avb_decoder_get_last_error(dec) ? avb_decoder_get_last_error(dec)
                                        : "unknown");
    avb_decoder_close(dec);
    return 1;
  }
  ...
  std::printf("Media Foundation %s decode smoke passed (%dx%d, codec '%s')\n",
              avb_video_codec_name(codec), info.video.width, info.video.height,
              actual);
  avb_decoder_close(dec);
  return 0;
}
...
static int smoke_ivf_output_formats(const char *path) {
  ...
  for (avb_pixel_format format : formats) {
    ...
    if (avb_decoder_open(&dec, path, &opts) != AVB_OK) {
      std::fprintf(
          stderr, "open IVF output format %d failed: %s\n", (int)format,
          avb_decoder_get_last_error(dec) ? avb_decoder_get_last_error(dec)
                                          : "unknown");
      avb_decoder_close(dec);
      return 1;
    ...
    if (avb_decoder_read_video_frame(dec, &frame) != AVB_OK ||
        frame.format != format || !frame.data || frame.width <= 0 ||
        frame.height <= 0) {
...
static int smoke_audio_encoder(const char *input_path, const char *output_path,
                               avb_audio_codec codec, const char *expected_name,
                               const char *label) {
  avb_encoder_capabilities caps{};
  if (avb_encoder_probe_runtime_capabilities(AVB_BACKEND_MEDIAFOUNDATION,
                                             output_path, &caps) != AVB_OK ||
      caps.result == AVB_ERROR_BACKEND_NOT_AVAILABLE) {
    std::printf("SKIP: Media Foundation runtime is not available\n");
    return AVB_TEST_SKIP;
  }
...
static int smoke_ivf_video_encoder(const char *input_path,
                                   ...
                                   avb_video_codec codec, const char *fourcc,
                                   const char *label) {
  avb_encoder_capabilities caps{};
  if (avb_encoder_probe_runtime_capabilities(AVB_BACKEND_MEDIAFOUNDATION,
                                             output_path, &caps) != AVB_OK ||
  ...
  if (avb_encoder_open(&enc, output_path, &eopts) != AVB_OK) {
    std::printf(
        "SKIP: %s IVF encoder did not accept the test session: %s\n", label,
        avb_encoder_get_last_error(enc) ? avb_encoder_get_last_error(enc)
                                        : "unknown");
    avb_encoder_close(enc);
    avb_decoder_close(dec);
    return AVB_TEST_SKIP;
  ...
  int decode_res =
      smoke_video_decoder(output_path, codec, avb_video_codec_name(codec));
  if (decode_res != 0) {
    if (decode_res == AVB_TEST_SKIP)
      std::fprintf(stderr, "%s IVF output was not decodable\n", label);
    return 1;
  }
  if (smoke_ivf_output_formats(output_path) != 0)
    return 1;
...
static int smoke_native_video_encoder(const char *output_path,
  ...
  if (avb_encoder_probe_runtime_capabilities(AVB_BACKEND_MEDIAFOUNDATION,
                                             output_path, &caps) != AVB_OK ||
      caps.result != AVB_OK || !has_video_codec(caps, codec)) {
  ...
  for (int y = 0; y < height; ++y) {
    for (int x = 0; x < width; ++x)
      nv12[(size_t)y * width + x] = (unsigned char)(16 + ((x + y) % 220));
  }
  ...
  int decoded =
      smoke_video_decoder(output_path, codec, avb_video_codec_name(codec));
  if (decoded != 0)
    return decoded == AVB_TEST_SKIP ? 1 : decoded;
  std::printf("Media Foundation native D3D11 %s encode smoke passed\n", label);
  return 0;
}

static int smoke_native_ivf_decoder(const char *input_path, const char *label) {
  avb_decode_options dopts = avb_decode_options_default();
  dopts.backend = AVB_BACKEND_MEDIAFOUNDATION;
  dopts.enable_audio = 0;
  dopts.enable_video = 1;
  ...
  HRESULT device_hr = D3D11CreateDevice(
      ...
      &expected_device, &feature_level, nullptr);
  if (FAILED(device_hr)) {
  ...
  while (frames < 7 && avb_decoder_read_video_frame(dec, &frame) == AVB_OK) {
    if (frame.memory_type != AVB_VIDEO_MEMORY_EXTERNAL ||
        frame.external_type != AVB_VIDEO_EXTERNAL_D3D11_TEXTURE ||
        frame.hardware_device != AVB_HW_DEVICE_D3D11VA ||
        !frame.native_handle || frame.format != AVB_PIXEL_FORMAT_NV12) {
...
static int
smoke_native_source_decoder(const char *input_path, const char *label,
                            bool required = true,
  ...
  if (avb_decoder_open(&dec, input_path, &opts) != AVB_OK) {
    std::fprintf(required ? stderr : stdout,
                 required ? "open native %s decoder failed: %s\n"
  ...
  for (int pass = 0; pass < 2; ++pass) {
    avb_video_frame frame{};
    avb_result read = avb_decoder_read_video_frame(dec, &frame);
    if (read != AVB_OK || frame.memory_type != AVB_VIDEO_MEMORY_EXTERNAL ||
        frame.external_type != AVB_VIDEO_EXTERNAL_D3D11_TEXTURE ||
        frame.hardware_device != AVB_HW_DEVICE_D3D11VA ||
        frame.format != AVB_PIXEL_FORMAT_NV12 || !frame.native_handle) {
      std::fprintf(stderr,
...
static int smoke_native_multi_frame_hold(const char *input_path,
  ...
  for (int i = 2; i >= 0; --i) {
    auto *texture = static_cast<ID3D11Texture2D *>(frames[i].native_handle);
    ComPtr<ID3D11Device> texture_device;
    texture->GetDevice(&texture_device);
    D3D11_TEXTURE2D_DESC desc{};
    texture->GetDesc(&desc);
  ...
  avb_decoder_close(dec);
  std::printf("Media Foundation native D3D11 %s multi-frame hold passed\n",
              label);
  return 0;
}
...
static int smoke_native_mp4_roundtrip(const char *input_path,
  ...
  eopts.video.input_external_type = AVB_VIDEO_EXTERNAL_D3D11_TEXTURE;
  ...
  int frames = 0;
  for (;;) {
    avb_result write = avb_encoder_write_video(enc, &frame, frame.pts_sec);
    avb_decoder_release_video_frame(dec, &frame);
    if (write != AVB_OK) {
    ...
    ++frames;
    if (frames >= 7 || avb_decoder_read_video_frame(dec, &frame) != AVB_OK) {
      break;
    }
  }
  ...
  int decoded = smoke_video_decoder(output_path, output_codec,
                                    avb_video_codec_name(output_codec));
  if (decoded != 0)
    return decoded == AVB_TEST_SKIP ? 1 : decoded;
  std::printf(
...
int main(int argc, char **argv) {
  ...
  for (int i = 3; i < argc;) {
    ...
    if (std::strcmp(argv[i], "audio") == 0) {
      ...
      if (res == 0)
        ran_smoke = true;
      if (codec == AVB_VIDEO_CODEC_VP8)
        vp8_fixture = argv[i + 2];
      if (codec == AVB_VIDEO_CODEC_VP9)
        vp9_fixture = argv[i + 2];
      if (codec == AVB_VIDEO_CODEC_AV1)
        av1_fixture = argv[i + 2];
      i += 4;
    } else {
  ...
  if (!vp8_fixture.empty()) {
    int native_res =
        smoke_native_source_decoder(vp8_fixture.c_str(), "VP8 WebM", false);
    if (native_res == 0)
      ran_smoke = true;
  }
  if (!vp9_fixture.empty()) {
    int native_res =
        smoke_native_source_decoder(vp9_fixture.c_str(), "VP9 WebM", false);
    if (native_res == 0)
      ran_smoke = true;
  }
  if (!av1_fixture.empty()) {
    int native_res =
        smoke_native_source_decoder(av1_fixture.c_str(), "AV1 Matroska", false);
    if (native_res == 0)
      ran_smoke = true;
  }
#endif
  ...
  int pcm_res = smoke_audio_encoder(
      argv[1], pcm_out.c_str(), AVB_AUDIO_CODEC_PCM_S16, "pcm", "PCM_S16 WAV");
  if (pcm_res == 1)
    return 1;
  if (pcm_res == 0)
    ran_smoke = true;
  ...
  int pcm_f32_res =
      smoke_audio_encoder(argv[1], pcm_f32_out.c_str(), AVB_AUDIO_CODEC_PCM_F32,
                          "pcm_f32", "PCM_F32 WAV");
  if (pcm_f32_res == 1)
    return 1;
  if (pcm_f32_res == 0)
  ...
  int flac_res = smoke_audio_encoder(argv[1], flac_out.c_str(),
                                     AVB_AUDIO_CODEC_FLAC, "flac", "FLAC");
  if (flac_res == 1)
    return 1;
  if (flac_res == 0)
    ran_smoke = true;
  ...
  if (!vp8_fixture.empty()) {
    ...
    int vp8_res = smoke_ivf_video_encoder(vp8_fixture.c_str(), vp8_out.c_str(),
                                          AVB_VIDEO_CODEC_VP8, "VP80", "VP8");
    if (vp8_res == 1)
      return 1;
    if (vp8_res == 0)
      ran_smoke = true;
  ...
  if (!vp9_fixture.empty()) {
    ...
    int vp9_res = smoke_ivf_video_encoder(vp9_fixture.c_str(), vp9_out.c_str(),
                                          AVB_VIDEO_CODEC_VP9, "VP90", "VP9");
    if (vp9_res == 1)
      return 1;
    if (vp9_res == 0)
      ran_smoke = true;
  ...
  if (!av1_fixture.empty()) {
    ...
    int av1_res = smoke_ivf_video_encoder(av1_fixture.c_str(), av1_out.c_str(),
                                          AVB_VIDEO_CODEC_AV1, "AV01", "AV1");
    if (av1_res == 1)
      return 1;
    if (av1_res == 0)
      ran_smoke = true;
...
#ifdef _WIN32
  {
    ...
    std::string native_h264_out = std::string(argv[2]) + ".native.h264.mp4";
    native_res = smoke_native_video_encoder(native_h264_out.c_str(),
                                            AVB_VIDEO_CODEC_H264, "H264 MP4");
    if (native_res == 1)
      return 1;
    if (native_res == 0)
      ran_smoke = true;
    ...
    native_res = smoke_native_mp4_roundtrip(
        argv[1], native_roundtrip_out.c_str(), AVB_VIDEO_CODEC_H264, "H264");
    if (native_res != 0)
      return 1;
    ran_smoke = true;

    std::string native_hevc_out = std::string(argv[2]) + ".native.hevc.mp4";
    native_res = smoke_native_video_encoder(native_hevc_out.c_str(),
                                            AVB_VIDEO_CODEC_HEVC, "HEVC MP4");
    if (native_res == 1)
      return 1;
    if (native_res == 0)
      ran_smoke = true;
    ...
    native_res =
        smoke_native_mp4_roundtrip(argv[1], native_hevc_roundtrip_out.c_str(),
    ...
    if (native_res == 0) {
      ran_smoke = true;
      native_res = smoke_native_ivf_decoder(native_out.c_str(), "AV1");
      if (native_res != 0)
        return 1;
      if (smoke_native_multi_frame_hold(native_out.c_str(), "AV1 IVF") != 0) {
        return 1;
      }
    }
  }
